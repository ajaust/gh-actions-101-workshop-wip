...
ACTIONLINT_INSTALL_DIR="/opt/actionlint"
...
sudo mkdir -p "${ACTIONLINT_INSTALL_DIR}"
sudo tar -xzf "${DOWNLOAD_TEMP_FILE}" -C "${ACTIONLINT_INSTALL_DIR}"
echo 'export PATH=\"${ACTIONLINT_INSTALL_DIR}/bin:${PATH}\"' >> ~/.bashrc
